...
// Pako import will be removed after refactoring
...
class ZipEditor {
  ...
  static _generateCRCTable() {
    if (ZipEditor._crcTable) return;
    ZipEditor._crcTable = new Uint32Array(256);
    for (let i = 0; i < 256; i++) {
      let c = i;
      for (let k = 0; k < 8; k++) {
        c = ((c & 1) ? (0xEDB88320 ^ (c >>> 1)) : (c >>> 1));
      }
      ZipEditor._crcTable[i] = c;
    }
  }

  // Static initialization block to generate the table once when the class is loaded
  static {
    ZipEditor._generateCRCTable();
  }

  static _crc32(dataUint8Array) {
    // The table should be generated by the static block, but double-check.
    if (!ZipEditor._crcTable) { 
      console.warn("ZipEditor._crc32: CRC table not pre-generated. Generating now. This should not happen if static block executed correctly.");
      ZipEditor._generateCRCTable();
    }
    let crc = 0xFFFFFFFF;
    for (let i = 0; i < dataUint8Array.length; i++) {
      crc = (crc >>> 8) ^ ZipEditor._crcTable[(crc ^ dataUint8Array[i]) & 0xFF];
    }
   ...
   */
  async updateEntry(fileName, newContent) {
    ...
    const dataToCompress = new ByteArray(uncompressedData.buffer.slice(0, uncompressedData.byteLength)); // Ensure a copy for safety
    dataToCompress._length = uncompressedData.byteLength; // Set length explicitly

    if (entry.compressionMethod === 8) { // Deflate
      try {
        await dataToCompress.deflate(); // Uses 'deflate-raw' by default
        ...
        console.error(`ZipEditor.updateEntry: Error deflating data for "${fileName}". Native deflate error:`, err);
        return false;
      }
    } else if (entry.compressionMethod === 0) { // Store
      entry.modifiedData = uncompressedData; // Store uncompressed data directly (Uint8Array)
  ...
  _calculateCRC32(data) {
    // Calls the new static JavaScript-based CRC32 implementation
    return ZipEditor._crc32(data);
  }
   ...
   * @returns {Promise<string|ByteArray|Uint8Array|null>} The extracted data in the specified format, or null on error.
   */
  async extractEntry(entryOrFileName, outputFormat = 'string') {
    let entry;
    ...
    try {
      ...
      // 3. Decompress Data
      let uncompressedDataUint8Array;
      if (entry.compressionMethod === 8) { // Deflate
        try {
          // Create a ByteArray from the compressed data to use its inflate method
          const compressedByteArray = new ByteArray(compressedFileData.buffer.slice(compressedFileData.byteOffset, compressedFileData.byteOffset + compressedFileData.byteLength));
          compressedByteArray._length = compressedFileData.byteLength; // Set length explicitly

          await compressedByteArray.inflate(); // Use 'deflate-raw' by default
          uncompressedDataUint8Array = new Uint8Array(compressedByteArray.buffer, 0, compressedByteArray.length);

        } catch (err) {
          console.error(`ZipEditor.extractEntry: Error decompressing data for "${entry.fileName}". Native inflate error:`, err);
          return null;
        }
      } else if (entry.compressionMethod === 0) { // Store (no compression)
        uncompressedDataUint8Array = compressedFileData; // This is already a Uint8Array
      } else {
      ...
      if (uncompressedDataUint8Array && uncompressedDataUint8Array.length !== entry.uncompressedSize) {
        console.warn(`ZipEditor.extractEntry: Uncompressed data size (${uncompressedDataUint8Array.length}) does not match expected size (${entry.uncompressedSize}) for "${entry.fileName}". Data might be corrupt.`);
      }
      
      if (!uncompressedDataUint8Array) {
      ...
      switch (outputFormat) {
        case 'string':
          try {
            return new TextDecoder('utf-8').decode(uncompressedDataUint8Array);
          } catch (decodeError) {
            console.error(`ZipEditor.extractEntry: Error decoding data to UTF-8 string for "${entry.fileName}".`, decodeError);
            return null;
          }
        case 'bytearray':
          const newBa = new ByteArray(uncompressedDataUint8Array.buffer.slice(uncompressedDataUint8Array.byteOffset, uncompressedDataUint8Array.byteOffset + uncompressedDataUint8Array.byteLength));
          newBa._length = uncompressedDataUint8Array.length;
          return newBa;
        case 'uint8array':
          return uncompressedDataUint8Array;
        default:
          console.error(`ZipEditor.extractEntry: Unknown output format "${outputFormat}".`);
          return null;
      }
